...
-- TENANT SCHEMA INIT
--
-- Applied by TenantSchemaService into the schema named after an admin's slug,
-- never into public. Flyway keeps a separate history table inside each schema,
-- so tenants created later start at this file and catch up on their own.
--
-- Nothing here carries a tenant_id column: the schema *is* the tenant, and a
-- query can only ever see the one its connection is pointed at.
-- ============================================================================

-- 1. System units — reference data, seeded below, never mutated at runtime
